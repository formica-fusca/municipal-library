...
| | |
|---|---|
| The buffer | `packages/ddd-core/src/entity.ts` |
| The drain + sort | `packages/ddd-core/src/aggregate-root.ts` |
| Commit ordering | `packages/event-bus/src/unit-of-work.ts` |
| The bus | `packages/event-bus/src/in-memory-event-bus.ts` |
